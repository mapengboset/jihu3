...
public class RUserLogBeanImpl implements RUserLogBean{
	...
	/** 键值格式 */
	private static final String KEY = "Id:user_id";
	private static final SimpleDateFormat DATE_FORMAT = new SimpleDateFormat("yyMMddhhmmss");
	
	/** 键值格式 */
	private static final MessageFormat KEY_FORMAT = new MessageFormat("Object:ticket_user{0}");
	
	private static final MessageFormat KEY_FORMAT_CODE = new MessageFormat("Object:ticket_user_code{0}");
	
	private static final MessageFormat KEY_FORMAT_SETUT = new MessageFormat("Object:ticket_user_SETUT{0}");
	...
	public Long increment(long time) {
		...
		long index = this.redisTemplate.opsForValue().increment("Id:userid", 1L).longValue();
