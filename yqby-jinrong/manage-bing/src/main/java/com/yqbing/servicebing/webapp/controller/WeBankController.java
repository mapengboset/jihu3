...
//					ErrorCode.getErrInfo(ErrorCode.OTHERS));
	...
	@RequestMapping(value ={"/manageWB/webankopenpage"},method=RequestMethod.GET)
	public  ModelAndView  offeropenpage(){
		
		return new ModelAndView("wzBac/html/webank/webankoppen");
	}
	//返回当前开关状态
	@RequestMapping(value ={"/manageWB/queryWeBankopen"},method=RequestMethod.GET)
    public ResultModel queryWeBankopen(){
		ResultModel resultModel = ResultModel.success();
		try {
			WebankTaskRuleBean wb = weBankControService.queryWeBankopen();
			
			resultModel.setResult(wb);
		} catch (Exception e) {
    ...
    public ResultModel querytaskWeBank(String off){
		ResultModel resultModel = ResultModel.success();
		try {
			int  s = weBankControService.weBankOff(off);
			if(s < 0){
				resultModel = ResultModel.error(ErrorCode.OTHERS,
						ErrorCode.getErrInfo(ErrorCode.OTHERS));
				return resultModel;
