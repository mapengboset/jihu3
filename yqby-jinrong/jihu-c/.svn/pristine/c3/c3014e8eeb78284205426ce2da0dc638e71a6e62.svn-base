...
public class CommonService {
	...
	public  List<StoreAppRes> getTypeByApps(String type,UserInfoBean infoBean, int page, int size){
		...
		AppModule selectByCode = appModuleMapper.selectByCode(type);
		if(null != selectByCode){
			
			Long id = selectByCode.getId();
			type = id+"";
		}
	...
	//	if(null == obj){
			list = CpdList(list, map);//返回cpd app列表
	...
	//	List<BoxStoreAppRes> apps = new ArrayList<BoxStoreAppRes>();
		...
		List<UserLocalApp> typeByApps = userLocalAppMapper.queryUserId(infoBean.getId());
			
		    if(null != typeByApps && typeByApps.size() > 0){//
				Map<String, Object> map1 = new HashMap<String, Object>();
			    for (UserLocalApp taps : typeByApps) {
			    	map1.put(taps.getAppPack(), taps);
			    }
 ...
 public boolean getUserTask(Long id,String appPack){
		 
		 
	 UserAppTask appTask = null;
	    	try {
	    		 appTask = userAppTaskMapper.queryPackAndUserId(appPack, id);
	 ...
	 public void addUserActionLog(Long uid,String desces,String appPack ,String mac,Short opt){
		 ...
		 try {
			 ...
			 else if(opt == 2 || opt == 3){//登陆一天只存1次
						 criteria.andUidEqualTo(uid).andOccurtimeBetween(DateUtil.gettwelve(),DateUtil.getzero() );
			...
			e.printStackTrace();
			LOGGER.error("------------------------action 记录日志报错--------------"+e.getLocalizedMessage());
		}
