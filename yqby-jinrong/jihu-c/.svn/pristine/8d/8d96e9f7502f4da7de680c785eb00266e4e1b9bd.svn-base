...
public class HomePageServiceImpl extends CommonService implements HomePageService{
	 ...
	 @Override
		public String homeNew(String token, String device) {
			...
			try {
	        	...
	        	AppBannerExample picex = new AppBannerExample();
	        	 ...
	        	 AppModule am =  appModuleMapper.selectByCode("top_page_module");
...
//		map.put("body",res);
			...
			// example.setPage(" limt "+page+","+size);
			 ...
			 for (AppCoupon a : list) {
                ...
                if(StringUtils.isNotBlank(a.getQrcodeType())){
                	...
                	if(a.getIsApplyUrl().equals("0")){
                		  res.setIsApplyUrl(1+"");
                	}else{
                		 res.setIsApplyUrl(0+"");
                	}
                res.setOnclick(a.getLinkUrl());
        	    if(house != null){
        	    	res.setImg(house.getAppUrlImg());
        		    res.setDownLinkUrl(house.getAppUrl());
        	    }
   ...
   // 	ScreenService service = rScreenServiceData.get(infoBean.getId()+"");
	...
	@Override
	public String moreModelApp(String token, Integer page, Integer size,
			Integer id) {
		...
		if(infoBean == null){
		 ...
		 List<AppRecommend> ares =  appRecommendMapper.queryTagId(id+"",infoBean.getId(),page*size,size);
		 List<HomeAppRes> apps = new ArrayList<HomeAppRes>();
		 for (AppRecommend a : ares) {
	...
	@Override
	public String gameRank(String token, Integer page, Integer size, String type) {
		UserInfoBean infoBean = queryTokenByUser(token);
		if(infoBean == null){
			return NotifyUtil.error(ErrorCodeEnum.NOLOGIN, "用户不存在");
		 ...
		 AppModuleExample example = new AppModuleExample();
   	     ...
   	     example.createCriteria().andCodeEqualTo(type).andStatusEqualTo(0).andIsMoreEqualTo(0);
   	     example.setOrderByClause(" sort");
   	     List<AppModule> list = appModuleMapper.selectByExample(example);
		 List<AppRecommend> ares =  appRecommendMapper.queryTagRankId(list.get(0).getId()+"",infoBean.getId(),page*size,size);
	...
	@Override
	public String recommendNewApps(String token, Integer type) {
