...
import com.yqbing.servicebing.repository.database.dao.JianYuLogMapper;
...
public class AccountServiceImpl extends CommonService  implements AccountService{
	 ...
	 @Resource
	 private JianYuLogMapper jianYuLogMapper = null;
	 @Resource
	...
	public String downLog(String device, String appPack, String token,Integer type) {
		...
		if(type == 0){//开始下载 1下载完成 2.打开应用
			...
			sta = "打开应用";
			y = 6;
            //判断之前任务下载的时候是否可以给免费次数
			UserTaskLog taskLog = userTaskLogMapper.queryCPD(infoBean.getId(),appPack);
			if(taskLog != null){
				...
				UserAppTask uas = userAppTaskMapper.queryPackAndUserIdByStatus(appPack, infoBean.getId());
				if(uas != null){
					uas.setStatus(s);	
					uas.setUpdateTime(new Date());
					userAppTaskMapper.updateByPrimaryKey(uas);
				}
	...
	@Override
	public void jianyuCallBack(Integer type, String appPack, String token,
			String imei, String body) {
		...
		JianYuLog yuLog = null;
		try {
			
			 JSONObject json = new JSONObject(body);
			 int code = json.getInt("code");
			 yuLog =	 jianYuLogMapper.selectByUserIdAndAppPack(infoBean.getId(),appPack);
			 if(yuLog != null){//成功
				 if(code == 0){
					 Integer type2 = yuLog.getType();
					 
					 yuLog.setType(type2+type);
					 yuLog.setUpdateTime(new Date());
					 jianYuLogMapper.updateByPrimaryKeySelective(yuLog);
					 return ;
				 }
			 }
			 yuLog = new JianYuLog();
			 yuLog.setAppPack(appPack);
			 yuLog.setBody(body);
			 yuLog.setCode(code);
			 yuLog.setImei(imei);
			 yuLog.setType(type);
			 yuLog.setUserId(infoBean.getId());
			 yuLog.setCreateTime(new Date());
			 jianYuLogMapper.insertSelective(yuLog);
		} catch (Exception e) {
