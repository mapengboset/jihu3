...
public class WeBankController extends BaseController {
	...
	public ResultModel weback(
		...
		try {
			...
			Object wes = weBankService.queryList(validToken, version,
	...
	public ResultModel webanksub(@RequestParam(value = "token", required = true)String token, @RequestParam(value = "picUrl", required = true)String picUrl, @RequestParam(value = "name", required = true)String name,
			String version) {
		ResultModel resultModel = ResultModel.success();
		try {
			// 验证token
			...
			int wes = weBankService.webanksub(validToken, picUrl, name, version);
			if (wes < 1) {
				resultModel = ResultModel.error(ErrorCode.E1020203,
						ErrorCode.getErrInfo(ErrorCode.E1020203));
				return resultModel;
			}
			resultModel.setResult(wes);
		} catch (Exception e) {
	...
	@RequestMapping(value = { "/laxin/webankOff" }, method = RequestMethod.POST)
	@ApiOperation(value = "微众开关")
	public String webankOff(String data) {
		
		JSONObject webankOff = weBankService.webankOff(data,data);
		
		
		return new Gson().toJson(webankOff);
	}
