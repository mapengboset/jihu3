...
<script>
    ...
    var arr = [ //标题栏
                ...
                ,{field: 'invitedName', title: '门店名称', Width: 40}
                ,{field: 'picUrl', title: '授额图片',Width: 130,toolbar: '<div><a href="{{d.picUrl}}" target="_blank ">查看图片</a></div>'}
                ,{field: 'beInvited', title: '公司名称', width: 120}
                ,{field: 'phone', title: '提交用户', minWidth: 100}
                ,{field: 'createTime', title: '提交时间', minWidth: 100, sort: true}
                ,{field: 'money', title: '已启用额度', minWidth: 100}
                ,{field: 'isSuccessful', title: '是否成功', Width: 50,toolbar:'#issucc'}
            ];
    function search(){
        ...
        layui.use(['element','table','laydate'], function(){
            ...
            table.render({
                elem: '#wzTable'
                ,cellMinWidth: 80 
                ,url:'../manageWB/query'    //全局定义常规单元格的最小宽度，layui 2.2.1 新增
                ,cols: [arr]
...
<script type="text/html" id="issucc">
        {{#if (d.isSuccessful == 1) { }}
         <span>成功</span>
        {{# }else if(d.isSuccessful == 2){ }}
        <span>失败</span>
        {{# }else if(d.isSuccessful == 3){ }}
        <span>申请中</span>
        {{# }else if(d.isSuccessful == 4){ }}
        <span>成功</span>
        {{# } }}
