...
//		Map<String, Object> map =  (Map<java.lang.String, Object>) map1.get("data");
	...
	public CommonResult userAdd(String moble, String name,String idCard,String token,String appId) {
		...
		try {
			...
			if(byExample.size() >= 1){
				if(!byExample.get(0).getId().equals(user.getId())){
   ...
   /* public static <V> void main(String[] args) {
	...
	@Override
	public CommonResult delBaiMeiYun(String phone) {
		
		User user = userMapper.queryUserByMobile(phone);
		...
		return new CommonResult(ErrorCode.ALI_BAIMEIYUN_ERROE,"失败，用户不存在或者已经实名");
	}
