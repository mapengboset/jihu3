...
import com.yqbing.servicebing.repository.database.bean.TicketRechargeData;
...
import com.yqbing.servicebing.repository.database.dao.TicketRechargeDataExample;
import com.yqbing.servicebing.repository.database.dao.TicketRechargeDataExample.Criteria;
import com.yqbing.servicebing.repository.database.dao.TicketRechargeDataMapper;
...
public class AccountServiceImpl extends CommonService implements AccountService {
	...
	private static final Logger LOGGER = LoggerFactory.getLogger(AccountServiceImpl.class);
	 ...
	 @Value("${ali_equal_baimeiyun}")
	 private   String  aliequalbaimeiyun;
	 @Resource
	 ...
	 @Resource
	 private TicketRechargeDataMapper  ticketRechargeDataMapper;
...
//-----------------------------------------------------------------------------------------------------------------------------------------
	...
	//	String storeDaysum = sysDictMapper.getShopowner("store_daily_withdrawal_limit");//门店每日额度
 	   ...
 	   if(amount >infoBean.getWealth() ){//提现金额不能大于账户实际金额
	...
	public String myStoreInfo(String token) {
		...
		myStoreInfo.setWealth(infoBean.getWealth()==null?0:infoBean.getWealth());
		myStoreInfo.setAliAccount(aliAccount);
	...
	public String cashoutRegister(String token, String aliaccount, String name,
	    ...
	    map.put("userphone", infoBean.getPhone());
	    map.put("name", name);
	    map.put("idcard", card);
		
	    try {
	    	String data = SendUtils.doPost(aliequalbaimeiyun, map);
	    	 JSONObject JSONObject = new JSONObject(data);
			 int code = JSONObject.getInt("code");
			 if(code != 0){
				 
				 return NotifyUtil.error(ErrorCodeEnum.ALIEQUALBAIMEIYUN);
			 }
		} catch (Exception e) {
...
//返回空的时候 表示未绑定支付宝 
	...
	public String ifcashout(String token) {
		...
		try {
			...
			Map<String,String> map = new HashMap<String,String>();
		    map.put("userphone", infoBean.getPhone());
		    map.put("name", ue.getAliName());
		    map.put("idcard", ue.getIdcard());
	...
	@Override
	public String recharge(String token, String img) {
		...
		TicketRechargeData trdata = null;
		try {
			
			trdata = new TicketRechargeData();
			trdata.setUserId(infoBean.getId());
			trdata.setUserName(infoBean.getName());
			trdata.setUserRole(storeUser.getStatus()+"");
			trdata.setStoreId(storeUser.getStoreId());
			trdata.setStoreName(storeName);
			trdata.setRechargePic(img);
			trdata.setRegisterPhone(infoBean.getPhone());
			trdata.setSubmitTime(new Date());
			trdata.setStatus("submit");
			ticketRechargeDataMapper.insertSelective(trdata);
			return NotifyUtil.success();
	...
	@Override
	public String subrecharge(String token,Integer type) {
		...
		TicketRechargeDataExample dataExample = new TicketRechargeDataExample();
		Criteria criteria = dataExample.createCriteria();
		criteria.andUserIdEqualTo(infoBean.getId());
		if(type == 0 || type == null){
			
			dataExample.setOrderByClause(" submit_time desc");
		}
		
        if(type == 1){
        	criteria.andStatusEqualTo("recharge");
        	dataExample.setOrderByClause(" recharge_time desc");
		}
		List<TicketRechargeData> list = ticketRechargeDataMapper.selectByExample(dataExample);
